    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="p-4 space-y-4">
        {pending.map((mosque) => (
          <div
            key={mosque.id}
            className="bg-white dark:bg-white/5 rounded-2xl overflow-hidden shadow-sm border border-primary/5"
          >
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {mosque.menu.map((item) => (
                    <span
                      key={item}
                      className="px-3 py-1 bg-primary/5 text-primary rounded-lg text-xs font-bold border border-primary/10"
                    >
                      {item}
                    </span>
                  ))}
        ...
        {pending.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 text-slate-400 text-center px-8">
            <span className="material-symbols-outlined text-6xl mb-4 text-primary opacity-20">
              inventory_2
            </span>
